<template>
  <div class="post-entry">
    <section class="item-post d-flex" v-for="(item, index) in postList" :key="index">
      <nuxt-link :to="'/post/' + item._id" class="post-cover">
        <img class="poster-img" :data-src="item.posterUrl" style="height: 100%"
          :src="`${item.posterUrl}?imageView2/3/w/50|imageMogr2/blur/40x5`" :alt="item.title" />
        <!-- <progressive-img
          :src="item.posterUrl"
      ...
      <div class="post-wrap">
        ...
        <div class="post-meta">
          <span class="item-meta">
            <svg-icon name="eye" />{{ item.pv }} 热度
          </span>
          <span class="item-meta">
            <svg-icon name="heart_hollow" />{{ item.like }} 喜欢
          </span>
          <span class="item-meta">
            <nuxt-link :to="'/post/' + item._id">
              <svg-icon name="folder" />{{ item.tags && item.tags[0] }}
            </nuxt-link>
          </span>
        </div>
